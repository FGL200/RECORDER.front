...
const columns = [];
const id = Helper.getSearchParam("id");
...
(async () => {
  ...
  Helper.getById("delete-record-button").addEventListener("click", () => {

  });

  Helper.getById("new-entry-form").addEventListener("submit", function (e) {
    e.preventDefault();
    const data = Helper.getDataFromForm("new-entry-form");

    console.log({ data })
  });

  Helper.getById("update-record-form").addEventListener("submit", function (e) {
    e.preventDefault();
    const data = Helper.getDataFromForm("update-record-form");

    console.log({ data })

    if (Helper.getInvalids("update-record-form")) {
